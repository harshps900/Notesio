...
import { faEllipsisVertical, faPencil, faTrash, faShareNodes, faEye, faClock, faStar, faHamburger, faBars } from "@fortawesome/free-solid-svg-icons";
        ...
        >
            ...
            <div className="flex-grow overflow-hidden mb-4">
                ...
                {note.tags.length >0 && (
                    ...
                    <span backgroundColor={handelTagsColor(note.id)} className={` text-sm  leading-relaxed cursor-pointer font-mono top-4 p-2 ${textColorClass}   `}>{note.tags}</span>
                    </div>
                )}
                {/* <p className={`text-sm  leading-relaxed cursor-pointer p-2 bg-blue-100 `}>{note.tags}</p> {console.log(note.tags)} */}
            </div>
            ...
            <div className="mt-auto pt-3 border-t border-white/50 flex justify-between items-center">
                ...
                >
                    <FontAwesomeIcon
                        icon={faStar}
                        className={`${note.isFavourite ? `${isDark ? 'text-yellow-600' : 'text-yellow-500'}` : `${isDark ? 'text-gray-200' : 'text-gray-400'}`}`}
                    />
                </button>
                {permission === 'view' && (
                    <div className={`flex items-center text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded-full ${textColorClass}`}>
